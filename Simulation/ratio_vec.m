...
enum = [20:5:195,200:50:1950,2000:500:19500, 20000:5000:90000];%, 200000:50000:1950000, 2000000:500000:20000000];
...
for d = 1:num_days
    s_gbm = NaN(N,num_vol);
    s_gbm(1,:) = s0_gbm;
    Z_gbm = repmat(sqrt(V0),N,1)*sqrt(dt).*randn(N,num_vol);
    ...
    for j = 1:length(enum)
        ...
        r = log(s_gbm(idx(2:end),:)) - repmat(log(s_gbm(1,:)),enum(j),1);                      % Calculate the expanding return
        back_r = repmat(log(s_gbm(end,:)),enum(j),1) - log(s_gbm(idx(1:end-1),:));             % Calculate the backwards expanding return
        
        var_gbm(j,:,d) = alpha*var(r./repmat([enum(j):-1:1]',1,num_vol),1) ...
            + (1-alpha)*var(back_r./repmat([1:enum(j)]',1,num_vol),1);
        fprintf('%sd = %.4d - j = %.3d',repmat(sprintf('\b'), 1, 18),d,j);
